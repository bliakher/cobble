...
**WINDOWS**: 
Make sure to copy all DLL files to output directory. Find them in SDL2/lib/

Build script `build.bat` is provided for Windows
...
Script builds the project with CMake with the `Release` configuration.
Then it copies the .dll files from the SDL2 library to the output directory.
It also copies the data directory to output.
